#!/usr/bin/env python3
"""
Minecraft Wiki MCP Server - stdio 版本
基于标准输入输出(stdio)传输的 MCP 服务器，用于与 Claude Desktop 和其他支持 stdio 的 MCP 客户端集成。

该服务器提供了与现有 SSE 版本相同的功能：
- Wiki 页面搜索
- 页面内容获取 
- 批量页面获取
- 页面存在性检查
...
使用方法:
- 直接运行: python mcp_server_stdio.py
- 通过 MCP 客户端连接: stdio 传输方式
"""
...
def load_config():
    ...
    try:
        ...
        return {
            ...
            "mcp_server": {
                "name": "Minecraft Wiki MCP (stdio)",
                "description": "基于stdio传输的MCP服务器，提供Minecraft Wiki内容访问工具",
                "version": "1.0.0",
                "transport": "stdio"
            },
            "logging": {
